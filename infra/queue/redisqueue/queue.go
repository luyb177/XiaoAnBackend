...
func NewRedisTaskQueue(rds *redis.Redis, keys queue.QueueKey) *RedisTaskQueue {
...
// Enqueue  task -> 待处理队列
func (q *RedisTaskQueue) Enqueue(ctx context.Context, task queue.Task) error {
...
func (q *RedisTaskQueue) Dequeue(ctx context.Context) (queue.Task, error) {
	// 堵塞 原子性
...
// Ack 处理中队列 task -> 删除
func (q *RedisTaskQueue) Ack(ctx context.Context, task queue.Task) error {
	payload, err := task.Payload()
	if err != nil {
		return err
	}
...
func (q *RedisTaskQueue) Retry(ctx context.Context, task queue.Task, delay time.Duration) error {
	payload, err := task.Payload()
	if err != nil {
		return err
	}
...
// MoveToDLQ  实现接口
func (q *RedisTaskQueue) MoveToDLQ(ctx context.Context, task queue.Task) error {
	payload, err := task.Payload()
	if err != nil {
		return err
	}
